...
// <---------------------------Custom security---------------------------------------->
		...
		http.securityContext().requireExplicitSave(false).and()
				...
				.authorizeHttpRequests().requestMatchers("/myAccount/**", "/myBalance", "/myCards", "/myLoans", "/user")
				.authenticated().requestMatchers("/contact", "/notices", "/register").permitAll().and().formLogin()
				.and().httpBasic();
